...
def test_gibbs_free_energy(orca_parametrized):
    """
    ...
    result = orca_parametrized.gibbs_free_energy(mol)
    ...
    # For AM1, adjust threshold as energies are less negative
    if orca_parametrized.functional == "AM1":
        assert result < -5.0  # AM1 energies are less negative
    else:
        assert result < -200.0

@pytest.mark.parametrize("orca_parametrized", ["dft", "am1"], indirect=True)
def test_entropy(orca_parametrized):
    """
    ...
    result = orca_parametrized.entropy(mol)
...
@pytest.mark.parametrize("orca_parametrized", ["dft", "am1"], indirect=True)
def test_molecular_volume(orca_parametrized):
    """
    ...
    result = orca_parametrized.molecular_volume(mol)
...
@pytest.mark.parametrize("orca_parametrized", ["dft", "am1"], indirect=True)
def test_enthalpy(orca_parametrized):
    """
    ...
    mol = AddHs(MolFromSmiles("C1=CC=CC=C1"))
    # Result is typically in Atomic Units (Hartree)
    result = orca_parametrized.enthalpy(mol)
...
@pytest.mark.parametrize("orca_parametrized", ["dft", "am1"], indirect=True)
def test_ch_bond_lengths(orca_parametrized):
    """
    ...
    c_h_bond_lengths: list[tuple[int, int, float]] = orca_parametrized.get_bond_lengths(mol, 'C', 'H')
...
@pytest.mark.parametrize("orca_parametrized", ["dft", "am1"], indirect=True)
def test_cc_bond_lengths(orca_parametrized):
    """
    ...
    c_c_bond_lengths: list[tuple[int, int, float]] = orca_parametrized.get_bond_lengths(mol, 'C', 'C')
